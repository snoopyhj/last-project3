...
public class LoginDAOImpl implements LoginDAO {
    ...
    public boolean InsertkakaoInfo(String id, String username, String password, String name, String email, String phone_number, String gender, String birthDate) {  
        ...
        jdbcTemplate.update(sql, id, username, encodedPassword, name, email, phone_number, gender, birthDate);
    ...
    public boolean deleteUser(String mail, String password) {
        ...
        if (storedPassword != null && passwordEncoder.matches(password, storedPassword)) {
            // 2. 비밀번호가 일치하면 삭제 쿼리 실행
            String sqlDelete = "DELETE FROM MEMBER WHERE mail = ?";
            int rowsAffected = jdbcTemplate.update(sqlDelete, mail);

            // 3. 삭제된 행이 있으면 true 반환
            return rowsAffected > 0;
        }

        // 4. 비밀번호가 일치하지 않으면 삭제 실패
        return false;
    }
}
