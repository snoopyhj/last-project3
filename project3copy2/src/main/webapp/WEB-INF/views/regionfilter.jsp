...
<body>
    ...
    <script>
        ...
        // JWT 토큰을 서버에서 가져오는 함수
        function fetchJwtToken() {
            return fetch("/get-jwt", {
                method: "GET",
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    ...
                    throw new Error("JWT 토큰을 가져오는데 실패했습니다.");
                }
                return response.json();  // 서버에서 받은 JWT 토큰 반환
            });
        }

        // JWT 토큰 유효성 검증을 서버에 요청하는 함수 (다른 프로젝트)
        function validateJwtToken(token) {
            return fetch("https://localhost:8443/validate-token?token=" + encodeURIComponent(token), {
                method: "GET",
                headers: {
                    "Content-Type": "application/json"
                }
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error("JWT 유효성 검증 실패: " + response.status);
                }
                return response.json();  // 검증 결과 JSON 데이터를 반환
            });
        }
                ...
                .then(data => {
                    ...
                    return validateJwtToken(token)
                        .then(validationResponse => {
                            alert("token: " + token);
                            alert("validationResponse.isValid: " + validationResponse.valid);
